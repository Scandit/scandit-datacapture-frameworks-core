...
public final class DeserializationLifeCycleDispatcher {
    ...
    private func snapshotObservers() -> [DeserializationLifeCycleObserver] {
        lock.lock()
        let snapshot = observers.compactMap { $0 as? DeserializationLifeCycleObserver }
        lock.unlock()
        return snapshot
    }

    func dispatchDataCaptureViewDeserialized(view: DataCaptureView?) {
        snapshotObservers().forEach {
    ...
    func dispatchAddModeToContext(modeJson: String) throws {
        let observersList = snapshotObservers()
