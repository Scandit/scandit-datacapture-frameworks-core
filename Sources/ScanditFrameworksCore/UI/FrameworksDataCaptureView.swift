...
public class FrameworksDataCaptureView: NSObject, FrameworksBaseView {
    ...
    public func dispose() {
        dispatchMain { [weak self] in
            ...
            if let zoomGestureListener = zoomGestureListener {
                self.view?.zoomGestures.forEach { $0.remove(zoomGestureListener) }
            }
    ...
    public func registerZoomGestureListener(_ listener: FrameworksZoomGestureListener) {
        ...
        DispatchQueue.main.async {
            self.view?.zoomGestures.forEach { $0.add(listener) }
        }
    }
    ...
    public func triggerZoomIn() {
        DispatchQueue.main.async {
            self.view?.zoomGestures.first?.triggerZoomIn()
        }
    }

    public func triggerZoomOut() {
        DispatchQueue.main.async {
            self.view?.zoomGestures.first?.triggerZoomOut()
        }
    }

    public func selectZoomLevel(_ zoomLevel: Float) -> Float {
        guard let control = view?.controls.first(where: { $0 is ZoomSwitchControl }) as? ZoomSwitchControl
        else { return -1.0 }
        return Float(control.selectZoomLevel(CGFloat(zoomLevel)))
    }
